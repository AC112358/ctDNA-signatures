...
fournuc_sub = subparsers.add_parser('get-fournucs', help = 'Write 4-nucleotide context file for a given genome.')
fournuc_sub.add_argument('--fasta-file','-fa', type = file_exists, required = True, help = 'Sequence file, used to find context of mutations.')
fournuc_sub.add_argument('--regions-file','-r', type = file_exists, required = True)
fournuc_sub.add_argument('--n-jobs','-j', type = posint, default = 1, help = 'Number of parallel processes to use. Currently does nothing.')
fournuc_sub.add_argument('--output','-o', type = valid_path, required = True, help = 'Where to save compiled corpus.')
fournuc_sub.set_defaults(func = MotifCorpusMaker.create_fournuc_file)
...
def write_dataset(
    ...
    dataset = None
    if corpus_model.lower() == "motif":
        dataset = MotifCorpusMaker.create_corpus(
            **shared_args, 
            weight_col = weight_col,
            motif_files = vcf_files,
...
dataset_sub.add_argument('--corpus-model', '-m', type = str, required=True, help='Which corpus model to use (e.g,, "motif", "sbs"), case insensitive.')
...
dataset_sub.add_argument('--kmer-file','-kmers', type = file_exists,default=None,
                         help = 'Pre-calculated k-nucleotide context file.')
